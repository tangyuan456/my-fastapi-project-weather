...
def update_meal_status(self, user_input: str, meal_type: str = "auto") -> dict:
    ...
    try:
        ...
        # 更新用户档案
        print(f"🔍 [update_meal_status内部] 开始更新用户档案...")
        user_nickname = self.get_current_user()
        print(f"🔍 [update_meal_status内部] 当前用户：{user_nickname}")

        if user_nickname and self.users.get(user_nickname):
            user_profile = self.users[user_nickname]
            print(f"🔍 [update_meal_status内部] 找到用户档案")

            # 获取当前餐次的计划用于比较
            food_plan = user_profile.get("daily_plan", {}).get("food", [])
            ...
            old_status = user_profile.get(status_field, "没吃")
            print(f"🔍 [update_meal_status内部] 更新字段：{status_field}，从'{old_status}'改为'吃了'")

            user_profile[status_field] = "吃了"

            # 记录详细用餐信息
            meal_details = {
                "meal": detected_meal,
                "user_input": user_input,
                "recommended_plan": current_meal_plan,
                "timestamp": current_time.isoformat()
            }

            # 保存到日常历史记录
            if "meal_history" not in user_profile:
                user_profile["meal_history"] = []
            user_profile["meal_history"].append(meal_details)

            user_profile["last_updated"] = current_time.isoformat()

            # 关键：检查save_profiles方法是否存在
            print(f"🔍 [update_meal_status内部] 检查save_profiles方法：{hasattr(self, 'save_profiles')}")

            if hasattr(self, 'save_profiles'):
                self.save_profiles()
                print(f"✅ [update_meal_status内部] 档案保存成功")
            else:
                print(f"❌ [update_meal_status内部] 找不到save_profiles方法！")
                # 尝试直接调用导入的save_profiles
                from 初次录入 import save_profiles as save_profiles_func
                save_profiles_func(self.users)
                print(f"✅ [update_meal_status内部] 使用导入的save_profiles保存成功")

            # 检查是否完成了所有餐次
            meal_status = {
                "早餐": user_profile.get("早餐状态", "没吃"),
                "午餐": user_profile.get("午餐状态", "没吃"),
                "晚餐": user_profile.get("晚餐状态", "没吃")
            }
            ...
            if len(completed_meals) == 3:
                ...
                remaining_meal = next(meal for meal, status in meal_status.items() if status == "没吃")
            ...
            print(f"❌ [update_meal_status内部] 用户档案不存在")
            return {
                "success": False,
                "message": "❌ 用户档案不存在，请先创建健康档案"
            }
...
def get_daily_plan(self, view_type: str = "current_meal") -> dict:
    ...
    try:
        ...
        if user_nickname and self.users.get(user_nickname):
            ...
            if "daily_plan" not in user_profile:
                return {
                    "success": False,
                    "message": "❌ 今天还没有生成健康计划"
                }

            daily_plan = user_profile["daily_plan"]
            ...
            if view_type == "current_meal":
                ...
                if current_meal_plan:
                    return {
                        ...
                        "meal_status": user_profile.get(f"{current_meal}状态", "没吃")
                    }
                else:
                    return {
                        "success": False,
                ...
                # 返回饮水计划
                drink_plan = user_profile.get("drink_plan", 8)
                current_drinks = user_profile.get("drink_number", 0)
                ...
                if movement_plan:
                    return {
                        ...
                        "exercise_status": user_profile.get("运动状态", "没运动")
                    }
                else:
                    return {
                        "success": False,
